<div align="center">
  <div style="width: 800px">
    <div align="center">
      <hr />
      <h1 align="center">첫걸음 타운</h1>
      <h4 align="center">메타버스 환경의 청소년 금융지식 향상 플랫폼</h4><br>
      <a href="https://zep.us/play/R5orpr" target="_blank">
        <img src="./assets/project-banner.png" width="30%" />
      </a>
      <br /><br />
      <div style="width: 550px">
        <p>
          <strong>청소년 금융지식을 향상</strong>을 목적으로 메타버스 환경에서 게임형식으로<br> 금융활동을 체험하며 리스크 없이 <strong>실패를 하며 배우는 금융교육</strong> 플랫폼.<br>소비 성향에 맞는 직업을 선택해 그에 맞는 금융활동을 하며 재화를 벌고 쓰며,<br> 플랫폼 내 금융도서관을 통한 금융지식도 향상시키는 금융 교육 플랫폼
        </p>
      </div>
      <br>
      <div align="center" style="width: 300px">
        <div align="left">
          <ul style="text-align: left; display: inline-block;">
            <li><strong>📅 개발 기간</strong> : 2025.10.14 ~ 2025.11.23 (6주)</li>
            <li><strong>💻 플랫폼</strong> : Web / Mobile / ZEP</li>
            <li><strong>👨‍👩‍👦‍👦 개발 인원</strong> : 4명</li>
            <li><strong>🏢 주관</strong> : K-해커톤13 (교육과학기술부 주관)</li>
          </ul>
        </div>
      </div>
      <hr />
      <h3>영상 포트폴리오 🔗</h3>
      <a href="https://youtu.be/sPQDfO4vVGY" target="_blank">
        <img src="./assets/video-portfoilo-thumbnail.png" width="80%" />
      </a>
    </div>
    <br><br><br><hr/>
    <h2 align="left">🔎 목차</h2>
    <div align="left">
      <h4><a href="#developers">✨ 팀 구성</a></h4>
      <h4><a href="#keyFeatures">🌠 주요 기능</a></h4>
    </div>
    <br><br><hr/>
    <h2 align="left"><a name="developers">✨ 팀 구성</a></h2>
    <div align="center">
      <table>
        <tr>
          <td width="20%" align="center">
            <a href="https://github.com/Jodndud" target="_blank">
              <img src="./assets/profiles/wooyeon_jo.jpg" width="140px" /> <br>
              <p valign="top">
                <strong>조우영(팀장)</strong><br>
                《기획, ZEP Script 개발》
            </a></p>
          </td>
          <td width="20%" align="center">
            <a href="https://github.com/dahyunsouth" target="_blank">
              <img src="./assets/profiles/dahyeon_nam.jpg" width="140px" /> <br>
              <p valign="top">
                <strong>이상욱</strong><br>
                《ZEP Script 개발, FE, 배포》
            </a></p>
          </td>
        </tr>
        <tr>
          <td width="168px" valign="top">
            <sub>
              - 기획<br>
              - ZEP UI 디자인<br>
              - ZEP Script 개발(외부 API 연동)
            </sub>
          </td>
          <td width="168px" valign="top">
            <sub>
              - 직업별 게임 FE개발<br>
              - ZEP Script 개발(ZEP내부 UI구현)<br>
              - AI기반 설문조사 기능 구현<br>
              - 서비스 내 기능 배포
            </sub>
          </td>
        </tr>
      </table>
      <table>
        <tr>
          <td width="20%" align="center">
            <a href="https://github.com/0w0n2" target="_blank">
              <img src="./assets/profiles/hyewon_lee.jpg" width="140px" /> <br>
              <p valign="top">
                <strong>신유빈</strong><br>
                《기획, 발표》
            </a></p>
          </td>
          <td width="20%" align="center">
            <a href="https://github.com/Me-in-U" target="_blank">
              <img src="./assets/profiles/mingyu_kim.jpg" width="140px" /> <br>
              <p valign="top">
                <strong>염아연</strong><br>
                《BE, FE》
            </a></p>
          </td>
        </tr>
        <tr>
          <td width="168px" valign="top">
            <sub>
              - 중간발표 및 최종발표<br>
              - 기획 배경 및 자료수집
            </sub>
          </td>
          <td width="168px" valign="top">
            <sub>
              - 메타버스 환경 맵 구조 기획<br>
              - 사용자 저장 데이터베이스 구축
            </sub>
          </td>
        </tr>
      </table>
    </div>
    <br><br><hr/>
    

  <h2 align="left"><a name="keyFeatures">🌠 주요 기능</a></h2>
  <div align="center">
    <table>
      <tbody align="center">
        <tr>
          <th style="text-align: center"> 메타버스 맵 </th>
          <th style="text-align: center"> 직업선택 </th>
        </tr>
        <tr>
          <td width="50%"><img width="100%" src="./assets/map.png" /></td>
          <td width="50%"><img width="100%" src="./assets/select_job.png" /></td>
        </tr>
      </tbody>
      <tbody align="center">
        <tr>
          <th style="text-align: center"> 게임 모달 </th>
          <th style="text-align: center"> 게임화면 </th>
        </tr>
        <tr>
          <td width="50%"><img width="100%" src="./assets/game_modal.png" /></td>
          <td width="50%"><img width="100%" src="./assets/game.png" /></td>
        </tr>
      </tbody>
    </table>
  </div>
  <br><br>

</div>
</div>
